...
from train_cnn_motif_latent_plant_loo import PerResidueCnn,RbdEmbeddingDataset,collate_batch,first_key,load_h5_features,resolve_path,setup_threads

def log(x): print('[cnn-fixed-all] '+x, flush=True)
def seed_all(seed):
...
def main():
    ...
    ap.add_argument('--ranking-loss-weight',type=float,default=0.0); ap.add_argument('--ranking-top-k',type=int,default=20); ap.add_argument('--ranking-margin',type=float,default=0.5)
